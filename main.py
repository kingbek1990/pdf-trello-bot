...
def parse_zayavka(text: str) -> dict:
    ...
    # Bo'lim — "Ariza Beruvchi Bo'limi" dan keyin
    ...
    # Mahsulotlar — jadval qatorlaridan olish
    # PDF matnida raqam + mahsulot nomi + dona + narx formatida bo'ladi
    mahsulotlar = []
    
    lines = text.split('\n')
    for line in lines:
        line = line.strip()
        # "1 Кресло для персонала KRISTOPS J153A dona 1 000 000 ..." formatini qidirish
        match = re.match(r'^\d+\s+(.+?)\s+dona\s+[\d\s]+', line, re.IGNORECASE)
        if match:
        ...
        # "1 Кресло для персонала KRISTOPS J153A шт ..." formatini qidirish
        match = re.match(r'^\d+\s+(.+?)\s+(?:шт|dona|дона)\s', line, re.IGNORECASE)
        if match:
            nom = match.group(1).strip()
            if len(nom) > 3:
                mahsulotlar.append(nom)

    # Agar yuqoridagi usul ishlamasa, boshqa usul
    if not mahsulotlar:
        # Butun matndan "N Mahsulot nomi dona" formatini qidirish
        pattern = re.findall(r'\d+\s+((?:[А-ЯA-Za-zа-яёЁ][^\n]*?))\s+(?:dona|шт|дона)\s', text, re.IGNORECASE)
        for nom in pattern:
            nom = nom.strip()
            # Keraksiz so'zlarni filtrlash
            skip_words = ['ariza', 'zayavka', 'byurtma', 'muddati', 'beruvchi', 'bo\'limi']
            if len(nom) > 3 and not any(w in nom.lower() for w in skip_words):
                mahsulotlar.append(nom)

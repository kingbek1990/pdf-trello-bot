...
def parse_zayavka(text: str) -> dict:
    ...
    # Mahsulotlar
    # Matnni bir qatorga keltirish (ikki qatorga tushgan nomlarni birlashtirish)
    text_oneline = re.sub(r'\n(?!\d)', ' ', text)
    
    mahsulotlar = []

    # Format 1: detal raqami bor (6+ raqamli)
    # "1 94732659 ROD-FRT S/D LK CYL Dona ..."
    detal_matches = re.findall(
        r'(?<!\d)(\d{1,2})\s+\d{6,}\s+(.+?)\s+[Dd]on[ae]\s',
        text_oneline
    )
    if detal_matches:
        seen = set()
        for _, nom in detal_matches:
            nom = nom.strip()
            if len(nom) > 2 and nom not in seen:
                seen.add(nom)
                mahsulotlar.append(nom)

    # Format 2: detal raqamisiz
    # "1 Кресло для персонала KRISTOPS J153A dona ..."
    if not mahsulotlar:
        simple_matches = re.findall(
            r'(?<!\d)(\d{1,2})\s+([А-ЯA-Za-zа-яёЁ][^\n]+?)\s+[Dd]on[ae]\s',
            text_oneline
        )
        skip = ['ariza', 'zayavka', 'byurtma', 'muddati', "bo'limi", 'xajmi']
        seen = set()
        for _, nom in simple_matches:
            nom = nom.strip()
            if len(nom) > 2 and nom not in seen and not any(w in nom.lower() for w in skip):
                seen.add(nom)
                mahsulotlar.append(nom)

    result['mahsulotlar'] = mahsulotlar
    logger.info(f"Topilgan mahsulotlar ({len(mahsulotlar)} ta): {mahsulotlar}")
    return result

def format_card_name(raqam, sana, bolim, tur, mahsulotlar):
    # Karta nomida faqat birinchi 2 ta
